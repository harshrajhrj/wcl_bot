        ...
        if((message.author.id === '531548281793150987' || message.member.hasPermission("MANAGE_ROLES")) && !(args[0] === 'trace')) {
            ...
            async function gsrun(cl) {
                ...
                if(new_data.length === 1) {
                    data = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${new_data[0]}/values/${args[1]}?majorDimension=ROWS&key=AIzaSyDUq4w3z35sS28BKWLdXSh32hlwUDDaD1Y`,options);
                }
                else if(new_data.length > 1) {
                    data = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${new_data[5]}/values/${args[1]}?majorDimension=ROWS&key=AIzaSyDUq4w3z35sS28BKWLdXSh32hlwUDDaD1Y`,options);
                }
                else {
