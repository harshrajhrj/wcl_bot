    ...
    execute : async (message, args) => {
        ...
        try {
            ...
            for(let clan of sortedClanRecords){
                ...
                row += `${rank.toString().padEnd(4," ")} ${clan.name.padEnd(15," ").substring(0, 15)} ${record.padEnd(5," ")} ${(clan.averageSD).toFixed(2).toString().padEnd(5," ")} ${(clan.averagePerDest).toFixed(2).toString().padEnd(6," ")} ${"0".padEnd(6," ")} ${"0".padEnd(7," ")}\n\n`;
                rank++;
            }
        } catch (error) {
